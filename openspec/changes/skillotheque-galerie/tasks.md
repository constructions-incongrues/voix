...
- [ ] 1.1 `git init` et premier commit — rien ne bouge dans `~/.claude/skills/` avant que le dépôt sache revenir en arrière
- [ ] 1.2 Créer `voix/` et `REGISTRE.md` avec les sept voix, leurs questions, leur état, et les refus motivés déjà instruits (Gorz, Ellul, Castoriadis, Mumford, Weil, Graeber, Lordon, Kropotkine)
- [ ] 1.3 Écrire `install.sh` : une boucle de liens symboliques `voix/<nom>` → `~/.claude/skills/<nom>`, réexécutable sans dommage

## 2. Rétrofit des deux voix existantes

- [ ] 2.1 Copier `guy-debord` et `steve-albini` depuis `~/.claude/skills/` vers `voix/`, sans modification, et commiter cet état de référence
- [ ] 2.2 Ajouter les quatre sections à `voix/guy-debord/SKILL.md` en extrayant sa table d'inversion (`SKILL.md:47-66`) vers `## Compétence` et `## Signaux` ; formuler `## Question` et `## Trace`
- [ ] 2.3 Idem pour `voix/steve-albini/SKILL.md` depuis sa table (`SKILL.md:49-64`)
