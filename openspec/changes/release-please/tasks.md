...
- [x] 3.4 Décider si le correctif du journal de la sentinelle — fusionné avant l'adoption, sujet sans type, donc invisible à l'outil — doit être mentionné à la main dans les notes de cette première publication. Constaté : il est **absent du `CHANGELOG.md` généré**, conséquence directe du garde-fou n°4 d'`adr/0002`. **Décidé : non.** L'absence est la conséquence assumée du garde-fou, et le `CHANGELOG` est un index des publications, non le journal de bord du dépôt — celui-ci est dans `openspec/changes/archive/`, où le correctif figure. L'ajouter à la main rétablirait l'autorité humaine sur un journal dérivé, exactement ce qu'`adr/0002` refuse pour le numéro de version
