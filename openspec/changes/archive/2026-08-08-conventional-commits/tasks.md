...
| non conformes, robot inclus | **1** — `chore(main): release 0.5.0`, scope hors jeu |
| non conformes, hors robot | **0** sur 14 |
...
- [x] 4.1 **1 non conforme sur 15 robot inclus, 0 sur 14 hors robot.** Mesurer la conformité — **en vérifiant les jeux fermés, non la seule forme du préfixe, et en écartant les commits d'auteur robot** :
      ```
      T='feat|fix|docs|refactor|test|chore'; S='voix|sentinelle|registre|plugin|specs|mesure'
      git log --format='%an%x09%s' <sha-adoption>..HEAD \
        | grep -v $'^[^\t]*\[bot\]\t' | cut -f2- \
        | grep -cvE "^($T)(\(($S)\))?!?: "
      ```
      **Correction d'une mesure antérieure :** la commande d'origine était `grep -cvE '^(feat|fix|…)(\([a-z]+\))?!?: '`. Elle accepte n'importe quel scope minuscule et ne vérifie donc pas le jeu fermé. Elle avait rendu « 0 non conforme », en manquant `chore(main): release 0.5.0` — dont le scope `main` est hors jeu. Le contrôle sous-vérifiait ; il est corrigé ici.
