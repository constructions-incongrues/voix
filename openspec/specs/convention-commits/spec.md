...
Ce qu'un message de commit doit satisfaire pour être admis, et ce que la convention n'a pas le droit d'écraser. Les quarante-et-un premiers commits du dépôt étaient des phrases françaises sans préfixe de type : lisibles par un humain, opaques à toute machine. Conventional Commits rend le niveau de version dérivable ; les garde-fous écrits ici empêchent la convention d'aplatir ce que les messages savaient déjà dire, à commencer par la correction d'une croyance antérieure. La table de correspondance décrit la configuration de l'outil de publication, qui fait autorité depuis `adr/0002`.
